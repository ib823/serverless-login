...
export async function POST(request: NextRequest) {
  ...
  const authCode = await popAuthCode(String(code));
  if (!authCode) {
    return NextResponse.json({ error: 'Invalid code' }, { status: 400 });
  if (authCode.client_id !== clientId || authCode.redirect_uri !== redirectUri) {
  const challengeVerify = createHash('sha256').update(String(codeVerifier)).digest('base64url');
  ...
  const accessToken = await signJWT({ sub: authCode.sub, scope: 'openid email' }, `${accessTokenTTL}s`);
  // Minimal rotating refresh token (store id + rotation in Redis)
